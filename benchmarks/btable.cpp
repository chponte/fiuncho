           ...
           const unsigned short order, double &elapsed_time, const int affinity)
{
    ...
    BitTable<uint64_t> table(order, dataset.cases_words, dataset.ctrls_words);
    ...
    if (order == 2) {
        ...
        for (auto reps = 0; reps < repetitions / 10; reps++) {
            for (auto snp = 1; snp < snp_count; snp++) {
                BitTable<uint64_t>::fill(
                    table, dataset.cases[0][0], dataset.ctrls[0][0], 3,
                    dataset.cases[snp][0], dataset.ctrls[snp][0]);
            }
        }
        // Measure time
        ...
        for (auto reps = 0; reps < repetitions / 10; reps++) {
            for (auto snp = order - 1; snp < snp_count; snp++) {
                BitTable<uint64_t>::fill(table, last.cases, last.ctrls,
                                         last.size, dataset.cases[snp][0],
                                         dataset.ctrls[snp][0]);
            }
        }
        // Measure time
